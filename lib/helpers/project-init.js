...
function setupApp(target, options) {
  if (target === 'app' || target === 'inRepoAddon') {
    fs.mkdirsSync('my-app');
    console.log('made my-app dir');
    process.chdir(path.resolve(tmpenv.tmpdir, 'my-app'));
    console.log('chdir: ',path.resolve(tmpenv.tmpdir, 'my-app'))
  }
  return Promise.resolve();
}
...
console.log('stepping into ' + tmp, existsSync(tmp))
